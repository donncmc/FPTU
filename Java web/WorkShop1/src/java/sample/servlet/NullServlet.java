...
public class NullServlet extends HttpServlet {
    ...
    private final String searchByStaffPage = "searchByStaff.jsp";
    private final String searchByUserPage = "searchByUser.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (cookie != null) {
                ...
                String password = cookie[cookie.length - 1].getValue();
                tbl_UserDAO dao = new tbl_UserDAO();
                int result = dao.checkLogin(username, password);
                if (result == 2) {
                    url = searchByStaffPage;

                    HttpSession session = request.getSession();
                    session.setAttribute("USERNAME", dao.getFullname(username, password));
                } else if (result == 0) {
                    url = searchByUserPage;
